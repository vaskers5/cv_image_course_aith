...
class SSLVisualClassifier(nn.Module):
    def __init__(self, vision_model_name, num_classes=8):
        ...
        self.classifier = nn.Linear(self.vision_model.config.hidden_size * 2, num_classes)
...
class VisualClassifier(nn.Module):
    def __init__(self, model_name, num_classes, model_configs,hidden_dim=1024, use_ssl=False):
        super().__init__()
        ...
        if model_name == "ViT":
            self.vision_model = ViTModel.from_pretrained(config["ssl_args"]["vision_model_name"])
            if use_ssl:
            ...
            if use_ssl:
                ...
                self.cnn.load_state_dict({k.replace("cnn.", ""): v for k, v in alex_weights.items() if k.startswith("cnn")})
                self.fc6.load_state_dict({k.replace("fc6.", ""): v for k, v in alex_weights.items() if k.startswith("fc6")})
